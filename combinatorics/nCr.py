...
def ncr_calculator(n, r):
    if r < 0 or r > n:
        return 0
    # Use symmetry to reduce calculations: nCr = nC(n-r)
    r = min(r, n - r)
    if r == 0:
        return 1
    
    # Calculate nCr as (n * (n-1) * ... * (n-r+1)) / (1 * 2 * ... * r)
    numerator = 1
    denominator = 1
    for i in range(r):
        numerator *= (n - i)
    ...
    while True:
        ...
        if inp == "ok":
            ...
            if len(form.form_list) == 2:
                form.form_list.append(f"nCr is:") 
                form.form_list.append(f"{nCr}")
            else:
                form.form_list[-2] = f"nCr is:"
                form.form_list[-1] = f"{nCr}"
