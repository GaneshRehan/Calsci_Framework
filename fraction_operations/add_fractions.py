...
def add_fractions_calculator(fractions):
    """Add all fractions: (a/b) + (c/d) + ..."""
    if not fractions:
        return 0, 1
...
def add_fractions(db={}):
    ...
    while True:
        ...
        if inp == "ok":
            ...
            result = add_fractions_calculator(fractions)
            if len(form.form_list) == 2:
                form.form_list.append(f"Sum is:")
                form.form_list.append(fraction_to_string(*result))
            else:
                form.form_list[-2] = f"Sum is:"
