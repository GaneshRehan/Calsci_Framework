class Form:
    def __init__(self, rows=7, menu_cursor=0, menu_display_position=0, input_list={"inp_0":" ", "inp_1":" ", "inp_2":" ", "chk_0": False}, form_list=["label_0", "inp_0", "label_1", "inp_1", "label_2", "inp_2", "chk_0"], input_cursor=0, input_display_position=0, input_cols=19):
        self.rows = rows
    ...
    def update_buffer(self, inp):
        if inp == "nav_d":
            ...
            current_field = self.form_list[self.menu_cursor]
            if "inp_" in current_field:
                self.refresh_rows = (self.menu_cursor - self.menu_display_position, self.menu_cursor - self.menu_display_position + 1)
                if inp == "nav_r":
                    self.input_cursor += 1
                    if self.input_cursor == len(self.input_list[current_field]):
                        self.input_cursor = 0
                        self.input_display_position = 0
                    ...
                    if self.input_cursor < 0:
                        self.input_cursor = len(self.input_list[current_field]) - 1
                        self.input_display_position = len(self.input_list[current_field]) - self.input_cols
                        if self.input_display_position < 0:
                            self.input_display_position = 0
                    elif self.input_cursor < self.input_display_position:
                        self.input_display_position -= 1
                    if inp == "nav_b" and self.input_cursor != len(self.input_list[current_field]) - 1:
                        self.input_list[current_field] = self.input_list[current_field][:self.input_cursor] + self.input_list[current_field][self.input_cursor + 1:]
                        if len(self.input_list[current_field]) > self.input_cols and len(self.input_list[current_field][self.input_display_position:]) < self.input_cols:
                            self.input_display_position = len(self.input_list[current_field]) - self.input_cols
                        elif len(self.input_list[current_field]) <= self.input_cols:
                            self.input_display_position = 0
                elif inp == "AC":
                    self.input_list[current_field] = " "
                    self.input_cursor = 0
                    self.input_display_position = 0
                else:
                    if len(inp) > 1:
                        for chr in inp:
                            self.input_list[current_field] = self.input_list[current_field][:self.input_cursor] + chr + self.input_list[current_field][self.input_cursor:]
                            self.input_cursor += len(chr)
                            if self.input_cursor == self.input_display_position + self.input_cols:
                                self.input_display_position += 1
                    else:
                        self.input_list[current_field] = self.input_list[current_field][:self.input_cursor] + inp + self.input_list[current_field][self.input_cursor:]
                        self.input_cursor += len(inp)
                        if self.input_cursor == self.input_display_position + self.input_cols:
                            self.input_display_position += 1
                    self.input_list[current_field] = self.input_list[current_field].rstrip() + " "
            elif "chk_" in current_field and inp == "ok":
                self.input_list[current_field] = not self.input_list[current_field]
                self.refresh_rows = (self.menu_cursor - self.menu_display_position, self.menu_cursor - self.menu_display_position + 1)
    ...
    def get_display_field(self, field):
        if "chk_" in field:
            label = "Option " + field.split("_")[1]
            return f"[{'X' if self.input_list[field] else ' '}] {label}"
        elif "inp_" in field:
            return self.input_list[field][self.input_display_position:self.input_display_position + self.input_cols]
        else:
            return field
...
def test2():
    ...
    while True:
        ...
        for i in range(form.act_rows()):
            print(f"{i}: {form.get_display_field(form.buffer()[i])}")
