...
def scientific_calculator(db={}):
    display.clear_display()
    json_file = "/database/scientific_calculator_app_list.json" 
    with open(json_file, "r") as file:
        data = json.load(file)
    
    scientific_calculator_apps_list = []
    
    

    for app in data:
        if app["visibility"]:
            scientific_calculator_apps_list.append(app["name"])

    scientific_calculator_apps_list.sort()
    
    menu.menu_list=scientific_calculator_apps_list
