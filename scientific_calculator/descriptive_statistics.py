...
def descriptive_statistics(db={}):
    display.clear_display()
    json_file = "/database/descriptive_statistics_app_list.json" 
    with open(json_file, "r") as file:
        data = json.load(file)
    
    descriptive_statistics_app_list = []
    

    for app in data:
        if app["visibility"]:
            descriptive_statistics_app_list.append(app["name"])

    descriptive_statistics_app_list.sort()

    menu.menu_list=descriptive_statistics_app_list
    menu.update()
    ...
    try:
        while True:
            ...
            if inp_menu == "back":
                ...
                current_app[1] = "descriptive_statistics"
                break
