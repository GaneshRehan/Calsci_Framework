...
def unit_converter(db={}):
    display.clear_display()
    json_file = "/database/unit_converter_app_list.json" 
    with open(json_file, "r") as file:
        data = json.load(file)
    
    unit_converter_app_list = []
    

    for app in data:
        if app["visibility"]:
            unit_converter_app_list.append(app["name"])

    unit_converter_app_list.sort()

    menu.menu_list=unit_converter_app_list
    menu.update()
