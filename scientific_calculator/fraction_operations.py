...
def fraction_operations(db={}):
    display.clear_display()
    json_file = "/database/fraction_operations_app_list.json" 
    with open(json_file, "r") as file:
        data = json.load(file)
    
    fraction_operations_app_list = []
    

    for app in data:
        if app["visibility"]:
            fraction_operations_app_list.append(app["name"])

    fraction_operations_app_list.sort()

    menu.menu_list=fraction_operations_app_list
    menu.update()
    ...
    try:
        while True:
            ...
            if inp_menu == "back":
                ...
                current_app[1] = "fraction_operations"
                break
